export const stats = [
    {
      number: "1.4k+",
      text: "Communities Formed",
      icon: "/source/images/icon-communities.svg" // Replace with actual image path
    },
    {
      number: "2.7m+",
      text: "Messages Sent",
      icon: "/source/images/icon-messages.svg" // Replace with actual image path
    }
  ];
  
  export const data = [
    {
      title: "Grow Together",
      description: "Generate meaningful discussions with your audience and build a strong, loyal community. Think of the insightful conversations you miss out on with a feedback form.",
      image: "/source/images/illustration-grow-together.svg",
    },
    {
      title: "Flowing Conversations",
      description: "You wouldn't paginate a conversation in real life, so why do it online? Our threads have just-in-time loading for a more natural flow.",
      image: "/source/images/illustration-flowing-conversation.svg",
    },
    {
      title: "Your Users",
      description: "It takes no time at all to integrate Huddle with your app's authentication solution. This means, once signed in to your app, your users can start chatting immediately.",
      image: "/source/images/illustration-your-users.svg",
    }
  ];

  export const locations = [
    
    {
      icon: "/source/images/icon-phone.svg",
      text: "+1-543-123-4567"
    },
    {
      icon: "/source/images/icon-email.svg",
      text: "example@huddle.com"
    }
  ];